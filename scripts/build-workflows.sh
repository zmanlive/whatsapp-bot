...
MERGE_JS=$(cat "$JS/merge-context.js")
...
BUILD_MSG_SQL_JS=$(cat "$JS/build-msg-sql.js")
...
jq \
  ...
  --arg build_msg_sql "$BUILD_MSG_SQL_JS" \
  '.nodes[] |= (
    if .name == "Init Security"      then .parameters.jsCode = $init          else . end |
    if .name == "Merge Context"      then .parameters.jsCode = $merge         else . end |
    ...
    if .name == "Build Msg SQL"      then .parameters.jsCode = $build_msg_sql else . end
  )' \
