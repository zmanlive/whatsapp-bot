...
if (fs > 25 * 1024 * 1024)
  return [{ json: { action: 'send', to: from, text: 'Audio too large (max 25MB).' } }];
if (dur > 300)
  return [{ json: { action: 'send', to: from, text: 'Audio too long (max 5 min).' } }];
if (!mUrl)
  return [{ json: { action: 'send', to: from, text: 'Audio unavailable. Try again.' } }];
